...
public class Https extends Servlets {
	private static final String BAND_WIDTH_AVG = "bandWidthAvg";
	private static final String BAND_WIDTH_COUNT = "bandWidthCount";
	...
	throws IOException {
		...
		if (headOnly) {
			...
			if (from >= 0) { //partial
				...
				if(request.getRequestURI()!=null && (request.getRequestURI().toUpperCase().endsWith(".JPG") || request.getRequestURI().toUpperCase().endsWith(".PNG"))){
					ArrayBlockingQueue<Double> bandWidthCount=(ArrayBlockingQueue<Double>)request.getSession().getAttribute(BAND_WIDTH_COUNT);
					if(bandWidthCount==null){
						bandWidthCount=new ArrayBlockingQueue<Double>(10);
						request.getSession().setAttribute(BAND_WIDTH_COUNT,bandWidthCount);
					}
					double timeSpent=end-start;
					if(timeSpent>=1){
						double savg=data.length/timeSpent;
						log.info("write time ["+timeSpent+"] data length["+data.length+"] avg["+savg+"]");
						if(bandWidthCount.size()>=10){
							bandWidthCount.poll();
						}						
						bandWidthCount.add(savg);
						Iterator iterate=bandWidthCount.iterator();
						Double sum=0D;
						for (Double long1 : bandWidthCount) {
							sum+=long1;
						}
						Double avg=sum/bandWidthCount.size(); // ms/byte
						avg=avg/1024*1000; // s/kbyte
						request.getSession().setAttribute(BAND_WIDTH_AVG,avg);
						log.info("avg ["+avg+"]");
					}
				}
			}
